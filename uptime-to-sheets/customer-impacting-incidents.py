...
import pytz
"""
    This script calculates the Customer Impacts Incidents from uptime monitoring
...
# Custom range
# window_start = datetime(2025, 9, 1, 0, 0, 0, tzinfo=timezone.utc)
# window_end = datetime(2025, 9, 30, 23, 59, 59, tzinfo=timezone.utc)
# month = window_start.strftime('%B %Y')
    

def get_customer_impacting_incidents():
    """Get Customer Impacting Incidents for the past month"""
    ...
    for monitor in data['monitors']:
        ...
        for log in monitor.get('logs', []):
            ...
            if log['type'] == 1 and window_start <= log_time < window_end:
    ...
    return num_failures, incident_sheet_url
...
def update_incident_report(incident_summaries):
    ...
    if not incident_summaries:
        ...
        return
    incident_sheet_name = f"CIFS Summaries {month}"

    # Open (or create) the incident summary sheet
    incident_sheet_name = f"CIFS Summaries {month}"
    try:
        incident_sh = gc.open("Customer Impacting Failures Summary Report")
        try:
...
if __name__ == "__main__":
    ...
    num_failures, incident_sheet_url = get_customer_impacting_incidents()
    row = [month, num_failures, incident_sheet_url]
