...
def get_weekly_window():
    """Returns the start and end dates for a weekly window (past 7 days)"""
...
window_start, window_end = get_weekly_window()
...
def main(window_start=window_start, window_end=window_end):
    ...
    sh = gc.open("TeamSight Workbook Weekly")
    # Create a new worksheet with the weekly date range
    sheet_title = f'TeamSight Weekly {window_start.replace("/","-")} to {window_end.replace("/","-")}'
