...
def get_monthly_window():
    """Returns the start and end dates for a monthly window (current month to date)"""
    today = datetime.now()
...
# Default to weekly window
window_start, window_end = get_monthly_window()
# For a fixed custom window, uncomment and set these:

# window_start = datetime(2025, 9, 1, 0, 0, 0, tzinfo=timezone.utc).strftime('%Y/%m/%d')
# window_end = datetime(2025, 9, 30, 23, 59, 59, tzinfo=timezone.utc).strftime('%Y/%m/%d')
...
def main(window_start=window_start, window_end=window_end):
    ...
    sh = gc.open("TeamSight Workbook Monthly")

    # Create a new worksheet with the monthly date range
    # Create a new worksheet with just the month name
    month_name = datetime.strptime(window_start, '%Y/%m/%d').strftime('%B %Y')
    sheet_title = f'TeamSight Monthly {month_name}'
